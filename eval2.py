...
import numpy as np
from model.eval import batch_level_evaluation, evaluate_entity_level_using_knn_2
...
def main(main_args):
    ...
    if dataset_name == 'streamspot' or dataset_name == 'wget':
        ...
        with torch.no_grad():
            ...
            x_train = np.concatenate(x_train, axis=0)[:50000]
            ...
            print(f'y_num {len(result_x_test)}')
            print(f'dimension {len(result_x_test[0])}')
            test_auc, test_std, _, _ = evaluate_entity_level_using_knn_2(dataset_name, x_train, result_x_test,
                                                                    result_y_test)
