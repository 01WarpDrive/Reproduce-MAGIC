...
metadata = {
    ...
    'optc_zeek':{
        'train': [
            'train_network_0922_1.json',
            'train_network_0922_2.json',
            'train_network_0922_3.json',
            'train_network_0922_4.json',
            'train_network_0922_5.json'
        ],
        'test': [
            'test_network_0922_ecarbro.json'
        ]
    }
}
    ...
    for file in os.listdir('../data/{}/'.format(dataset)):
        if 'json' in file and not '.txt' in file and not 'names' in file and not 'types' in file and not 'metadata' in file:
            ...
            for line in tqdm(f):
                ...
                entitiy_type = event['object']

                try:
                    actor_id = event['src_ip_port']
                    id_nodetype_map[actor_id] = entitiy_type
                    object_id = event['dest_ip_port']
                    id_nodetype_map[object_id] = entitiy_type
                    id_nodename_map[actor_id] = actor_id
                    id_nodename_map[object_id] = object_id
                except KeyError:
                    continue
    ...
    for key in metadata[dataset]:
        for file in metadata[dataset][key]:
            ...
            for line in tqdm(f):
                event = json.loads(line)
                try:
                    srcId = event['src_ip_port']
                    srcType = id_nodetype_map[srcId]
                    dstId = event['dest_ip_port']
...
def read_graphs(dataset):
    ...
    for file in metadata[dataset]['test']:
        ...
        with open(f'../data/{dataset}/node_list.txt', 'a') as file:
            for id in node_list:
                file.write(id)
                file.write('\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="optc_zeek")
    args = parser.parse_args()
    if args.dataset not in ['optc_zeek']:
